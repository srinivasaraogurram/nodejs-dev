...
NVM use for switching between different versions of Node Versions
...
# run the command "nvm" in the terminal,
it will provide the different opitions
...
If one wants to install specific version of nodejs say 12.18
```
nvm install 12.18.0
```
![Showing Node Verzion 12.18.0](image-1.png)

Switch to other version - say 14.x
![alt text](image-2.png)

Switch to latest version use the command with out version
```
nvm install node
```
...
# if node is already used and would like to switch between versions use command 'nvm use node' for latest version | nvm use 12.18.0 to use the specific version'
# if want to run specifc file with specific version or use shell with specific version

nvm exec 4.2 node --version  

# to swtich back to other version use command 'nvm use 14.15.1`
nvm exec 4.2 node -version
